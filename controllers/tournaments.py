...
class TournamentsController(AbstractController):
    ...
    def index(self):
        ...
        self.tournament_view.display_message_to_user("Nous n'avons trouvé aucun tournoi dans la base de données")
        return None
    ...
    def select_tournament(self):
        ...
        if existing_tournament:
            ...
            if existing_tournament["players"]:
                for existing_player in existing_tournament["players"]:
                    ...
                    player.player_id = self.players_db.get_player_id(
                        existing_player["first_name"], existing_player["last_name"],
                    )
            ...
            tournament.tournament_id = existing_tournament.doc_id
            self.tournament_view.display_message_to_user(f"Le tournoi {tournament.name} a correctement été trouvé et importé !")
            return tournament
        else:
            self.tournament_view.display_message_to_user("Aucun tournoi n'a pu être trouvé")
            return None

    def print_tournament_players(self, tournament: Tournament):
        self.player_view.print_players_tournament_ranking(tournament.sort_players_by_score())
        ...
        if tournament.current_round <= 4:
            ...
            if players_pairs:
                ...
                while confirm != "Y":
                    ...
                    self.round_view.display_message_to_user(f"{game_round.name} est en cours, Patientez jusque la fin des matchs avant d'inscrire les résultats")
                    self.round_view.display_message_to_user("Si vous quittez celle-ci, la ronde sera supprimée")
                    user_choice = self.round_view.get_user_choice(2)
                    if user_choice == 0:
                        self.round_view.display_message_to_user("Si vous quittez maintenant la ronde, celle-ci sera supprimée")
                ...
                self.round_view.display_message_to_user("Oula, quelque chose s'est mal passé, merci de recommencer")
                return
        else:
    ...
    def start_round(self, game_round, players_pairs, tournament, index):
        for first_player, second_player in players_pairs:
            ...
            match = Match(first_player.serialize_match_player, second_player.serialize_match_player, first_player_score, second_player_score)
            game_round.matchs.append(match)
            index += 1
        tournament.rounds.append(game_round)
        ...
        if tournament.current_round == 5:
            ...
            for player in tournament.sort_players_by_score():
                ...
                ladder += 1
            self.round_view.display_message_to_user(f"Le tournoi est TERMINÉ! Vous pouvez désormais afficher les résultats")
        else:
